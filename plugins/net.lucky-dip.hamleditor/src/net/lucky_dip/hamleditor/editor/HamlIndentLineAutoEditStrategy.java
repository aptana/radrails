 ...
 * 
 */
public class HamlIndentLineAutoEditStrategy implements IAutoEditStrategy {
	...
	private void autoIndentAfterNewLine(IDocument d, DocumentCommand c) {
		...
		try {
			...
			if (d instanceof IDocumentExtension3) {
				...
				lastPartitionType = id3.getDocumentPartitioner(
						HamlEditor.HAML_PARTITIONING).getContentType(
						c.offset - 1);
			}
			...
			if (lastPartitionType != null
					&& (lastPartitionType
							.equals(HamlPartitionScanner.HAML_CLASS)
							|| lastPartitionType
									.equals(HamlPartitionScanner.HAML_ID) || lastPartitionType
							.equals(HamlPartitionScanner.HAML_ELEMENT))) {
				buf.append("  ");
			}
