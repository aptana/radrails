...
public class HamlClassScanner extends RuleBasedScanner
{
	...
	public HamlClassScanner(IColorManager manager)
	{
		IToken classToken = new Token(new TextAttribute(manager.getColor(IHamlEditorColorConstants.HAML_CLASS)));
		IRule[] rules = new IRule[1];
		rules[0] = new WordPatternRule(new HamlWordDetector(), ".", "", classToken);
		setRules(rules);
	}
}
