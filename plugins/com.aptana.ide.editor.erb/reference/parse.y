...
yylex()
{
  ...
  retry:
    switch (c = nextc()) {
        ...
        // [KEL] May need to handle EXPR_* flags
        case '\n':
            switch (lex_state) {
                case EXPR_BEG:
                case EXPR_FNAME:
        ...
        case '*':
          if ((c = nextc()) == '*') {
              ...
              // [KEL] Not handling if/else-if cases
            ...
            if (was_bol()) {
                ...
                if (strncmp(lex_p, "begin", 5) == 0 && ISSPACE(lex_p[5])) {
                    for (;;) {
                        ...
                        if (c != '=') continue;
                        if (strncmp(lex_p, "end", 3) == 0 && (lex_p + 3 == lex_pend || ISSPACE(lex_p[3]))) {
                            break;
                        }
                    }
                    lex_p = lex_pend;
        ...
        // [KEL] Not handling two if statements
        case '`':
        ...
        // [KEL] feel like I'm missing something here :)
        case '?':
        ...
        // [KEL] Not handling code from "pushback" down
        case '&':
            if ((c = nextc()) == '&') {
                lex_state = EXPR_BEG;
                if ((c = nextc()) == '=') {
